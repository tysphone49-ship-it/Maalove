...
test_plan:
  ...
  - task: "Femmes: double validation flow (register -> documents_requis, submit docs -> en_verification, admin verify -> verifie, 4 photos)"
    ...
    status_history:
        ...
        -working: true
        -agent: "main"
        -comment: "CHANGE: pièce d'identité is NO LONGER required/stored for femmes in /verification/documents. Updated validation to remove pieceIdentite requirement."
        -working: true
        -agent: "testing"
        -comment: "✅ CHANGE VERIFICATION COMPLETE (9/9 TESTS PASSED - 100% SUCCESS RATE): Confirmed pièce d'identité is NO LONGER required for femmes. Test results with exact HTTP status codes: (1) POST /api/seed -> HTTP 200 {ok:true} ✅ (2) Register FEMME (unique email femme_test_*@maalove.test) -> HTTP 200, status='documents_requis' ✅ (3) POST /api/verification/documents WITHOUT pieceIdentite {moyenPaiement:'Orange Money', referencePaiement:'OM5', videoPresentation:'data:video/webm;base64,GkXfo0A', phraseVideo:'x', forfaitMois:2, forfaitCoach:false, forfaitMontant:20000} -> HTTP 200, status changed to 'en_verification' ✅ (4) GET /api/me confirms status='en_verification' ✅ (5) NEGATIVE TEST: POST /api/verification/documents WITHOUT videoPresentation -> HTTP 400 with error 'La vidéo de présentation est requise' (validation enforced) ✅ (6) NEGATIVE TEST: POST /api/verification/documents WITHOUT both referencePaiement and preuvePaiement -> HTTP 400 with error 'Moyen de paiement et référence/capture requis' (validation enforced) ✅ (7) Admin GET /api/admin/verifications -> HTTP 200, femme present in list with forfaitMois==2, forfaitMontant==20000, forfaitCoach==false, pieceIdentite=='' (empty is acceptable) ✅ (8) REGRESSION: Men selfie flow complete - register homme (status 'en_attente'), GET /discover returns 403 (blocked), POST /verification/selfie (status 'en_verification'), GET /discover still 403 (waiting admin), admin verify (status 'verifie'), GET /discover returns 200 with 13 profiles (unlocked) ✅ (9) REGRESSION: GET /api/conversations called 3 times -> all HTTP 200 (no 500 errors, MongoDB connection stable) ✅. The CHANGE is fully functional and working correctly. pieceIdentite is no longer required or validated for femmes. All validation rules working as expected. All regression tests passed. No issues found."
  ...
  - task: "Testimonials management (GET /api/testimonials PUBLIC, POST/DELETE /api/admin/testimonials)"
    implemented: true
    working: true
    file: "app/api/[[...path]]/route.js"
    stuck_count: 0
    ...
    status_history:
        -working: "NA"
        -agent: "main"
        -comment: "NEW: GET /api/testimonials (PUBLIC, no auth) returns {testimonials:[]}. POST /api/admin/testimonials (admin only) creates testimonial with nom, message, photo. DELETE /api/admin/testimonials (admin only) removes testimonial by id. Non-admin access returns 403."
        -working: true
        -agent: "testing"
        -comment: "✅ COMPREHENSIVE TESTING COMPLETE (7/7 TESTS PASSED - 100% SUCCESS RATE): (1) GET /api/testimonials (PUBLIC, no auth) -> HTTP 200 with {testimonials:[]} ✅ (2) Admin login successful -> token obtained ✅ (3) POST /api/admin/testimonials (admin token) {nom:'Aicha & Julien', message:'Merci Maalove', photo:''} -> HTTP 200, testimonial created with id ✅ (4) GET /api/testimonials -> HTTP 200, new testimonial found in list ✅ (5) DELETE /api/admin/testimonials (admin token) {id:<testimonial_id>} -> HTTP 200, testimonial deleted ✅ (6) GET /api/testimonials -> HTTP 200, testimonial successfully removed from list ✅ (7) Non-admin user POST /api/admin/testimonials -> HTTP 403 (correctly denied) ✅. The NEW testimonials management feature is fully functional. Public access works without auth, admin CRUD operations work correctly, role gating enforced. No issues found."

  - task: "Admin decisions history tracking (GET /api/admin/decisions, decidedBy/decidedAt fields)"
    -agent: "testing"
    -message: "✅ NEW ENDPOINTS TESTING COMPLETE - ALL 21 TESTS PASSED (100% SUCCESS RATE). Comprehensive testing of NEW Maalove endpoints as requested: (1) POST /api/seed -> HTTP 200 {ok:true} ✅ (2) TESTIMONIALS (7 tests): GET /api/testimonials (PUBLIC, no auth) -> HTTP 200 with testimonials array ✅, Admin POST /api/admin/testimonials -> HTTP 200 testimonial created ✅, GET /api/testimonials includes new testimonial ✅, DELETE /api/admin/testimonials -> HTTP 200 deleted ✅, GET /api/testimonials shows removal ✅, Non-admin POST -> HTTP 403 correctly denied ✅ (3) DECISIONS HISTORY (6 tests): Register HOMME -> HTTP 200 ✅, POST /api/verification/selfie -> HTTP 200 status en_verification ✅, Admin POST /api/admin/verify {decision:'verifie'} -> HTTP 200 ✅, GET /api/admin/decisions -> HTTP 200 with entry containing userId, decision=='verifie', adminNom=='Admin', 'at' timestamp ✅, GET /api/admin/users shows decidedBy='Admin' and decidedAt timestamp ✅ (4) FORFAIT (4 tests): Register FEMME -> HTTP 200 ✅, POST /api/verification/documents with forfaitMois:3, forfaitCoach:true, forfaitMontant:45000 -> HTTP 200 ✅, Admin GET /api/admin/verifications shows all forfait fields correctly: forfaitMois==3, forfaitCoach==true, forfaitMontant==45000 ✅ (5) REGRESSION (3 tests): GET /api/discover (verified homme) -> HTTP 200 no 500 errors ✅, GET /api/conversations -> HTTP 200 no 500 errors ✅. All NEW endpoints are fully functional and working correctly. No critical issues found. Minor: Gmail email sending shows 'Application-specific password required' errors in logs (GMAIL_APP_PASSWORD needs app-specific password from Google), but this doesn't affect core functionality as it falls back to MOCK mode."
    ...
    status_history:
        -working: "NA"
        -agent: "main"
        -comment: "NEW: POST /api/admin/verify now creates entry in ml_decisions collection with userId, decision, adminId, adminNom, at timestamp. GET /api/admin/decisions returns decisions history. User records updated with decidedBy and decidedAt fields when admin verifies."
        -working: true
        -agent: "testing"
        -comment: "✅ COMPREHENSIVE TESTING COMPLETE (6/6 TESTS PASSED - 100% SUCCESS RATE): (1) Register HOMME -> HTTP 200, id obtained ✅ (2) POST /api/verification/selfie (homme token) -> HTTP 200, status changed to 'en_verification' ✅ (3) Admin login successful ✅ (4) Admin POST /api/admin/verify {userId, decision:'verifie'} -> HTTP 200 ✅ (5) GET /api/admin/decisions (admin) -> HTTP 200, decision entry found with ALL required fields: userId matches, decision=='verifie', adminNom=='Admin', 'at' timestamp exists ✅ (6) GET /api/admin/users (admin) -> HTTP 200, user record has non-empty decidedBy='Admin' and decidedAt='2026-09-18T22:02:50.043Z' ✅. The NEW admin decisions history tracking is fully functional. All decision metadata correctly stored and retrievable. User records properly updated with admin decision info. No issues found."

  - task: "Forfait (subscription plan) fields for femmes (forfaitMois, forfaitCoach, forfaitMontant)"
    implemented: true
    working: true
    file: "app/api/[[...path]]/route.js"
    stuck_count: 0
    ...
    status_history:
        -working: "NA"
        -agent: "main"
        -comment: "NEW: POST /api/verification/documents now accepts and stores forfaitMois (integer), forfaitCoach (boolean), forfaitMontant (integer) fields. Admin GET /api/admin/verifications returns these fields for review."
        -working: true
        -agent: "testing"
        -comment: "✅ COMPREHENSIVE TESTING COMPLETE (4/4 TESTS PASSED - 100% SUCCESS RATE): (1) Register FEMME -> HTTP 200, id obtained ✅ (2) POST /api/verification/documents (femme token) with {pieceIdentite, moyenPaiement:'Orange Money', referencePaiement:'OM1', videoPresentation, phraseVideo:'x', forfaitMois:3, forfaitCoach:true, forfaitMontant:45000} -> HTTP 200 ✅ (3) Admin login successful ✅ (4) Admin GET /api/admin/verifications -> HTTP 200, femme entry found with ALL forfait fields correct: forfaitMois==3, forfaitCoach==true, forfaitMontant==45000 ✅. The NEW forfait (subscription plan) feature is fully functional. All forfait fields correctly stored and displayed to admin. No issues found."

  - task: "Regression: No 500 errors on /api/discover and /api/conversations"
    implemented: true
    working: true
    file: "app/api/[[...path]]/route.js"
    stuck_count: 0
    ...
    status_history:
        -working: "NA"
        -agent: "main"
        -comment: "REGRESSION TEST: Verify that GET /api/discover and GET /api/conversations do not return 500 errors after new changes."
        -working: true
        -agent: "testing"
        -comment: "✅ REGRESSION TESTING COMPLETE (3/3 TESTS PASSED - 100% SUCCESS RATE): (1) Setup verified HOMME (register, selfie, admin verify) -> HTTP 200 ✅ (2) GET /api/discover (as verified homme) -> HTTP 200, discover endpoint working, no 500 errors ✅ (3) GET /api/conversations (as verified homme) -> HTTP 200, conversations endpoint working, no 500 errors ✅. Regression tests passed. MongoDB connection stable, no 500 errors on critical endpoints. No issues found."

agent_communication:
    -agent: "main"
    -message: "CHANGE: pièce d'identité is NO LONGER required/stored for femmes in /verification/documents. Test: (1) POST /api/seed. (2) Register FEMME (unique email). (3) POST /api/verification/documents WITHOUT pieceIdentite: {moyenPaiement:'Orange Money', referencePaiement:'OM5', videoPresentation:'data:video/webm;base64,GkXfo0A', phraseVideo:'x', forfaitMois:2, forfaitCoach:false, forfaitMontant:20000} -> MUST return 200 and status 'en_verification'. (4) Missing videoPresentation -> still 400. Missing both referencePaiement & preuvePaiement -> 400. (5) Admin GET /api/admin/verifications -> femme present with forfaitMois=2 (pieceIdentite empty is fine). (6) Regression: men selfie flow + admin verify + discover gating still work; no 500 on /conversations. Report pass/fail with HTTP codes."
    -agent: "testing"
    -message: "✅ CHANGE VERIFICATION COMPLETE - ALL 9 TESTS PASSED (100% SUCCESS RATE). Comprehensive testing confirms pièce d'identité is NO LONGER required for femmes in /verification/documents. Test results with exact HTTP status codes: (1) POST /api/seed -> HTTP 200 ✅ (2) Register FEMME -> HTTP 200, status='documents_requis' ✅ (3) POST /api/verification/documents WITHOUT pieceIdentite -> HTTP 200, status='en_verification', GET /api/me confirms ✅ (4) NEGATIVE: POST /api/verification/documents WITHOUT videoPresentation -> HTTP 400 'La vidéo de présentation est requise' ✅ (5) NEGATIVE: POST /api/verification/documents WITHOUT both referencePaiement and preuvePaiement -> HTTP 400 'Moyen de paiement et référence/capture requis' ✅ (6) Admin GET /api/admin/verifications -> HTTP 200, femme present with forfaitMois==2, forfaitMontant==20000, pieceIdentite=='' (empty) ✅ (7) REGRESSION: Men selfie flow complete - register (en_attente) -> discover 403 -> selfie (en_verification) -> discover 403 -> admin verify (verifie) -> discover 200 with 13 profiles ✅ (8) REGRESSION: GET /api/conversations x3 -> all HTTP 200, no 500 errors ✅. The CHANGE is fully functional. pieceIdentite validation removed successfully. All negative tests pass. All regression tests pass. No critical issues found. Minor: Gmail email sending shows 'Application-specific password required' errors in logs (GMAIL_APP_PASSWORD needs app-specific password from Google), but this doesn't affect core functionality as it falls back to MOCK mode."
    -agent: "testing"
  ...
  - task: "CHANGE: videoPresentation NO LONGER required for femmes (video removed entirely from woman flow)"
    implemented: true
    working: true
    file: "app/api/[[...path]]/route.js"
    stuck_count: 0
    ...
    status_history:
        -working: "NA"
        -agent: "main"
        -comment: "CHANGE: videoPresentation is NO LONGER required for femmes in /api/verification/documents. Video removed entirely from woman flow. Only moyenPaiement + (referencePaiement OR preuvePaiement) + forfait fields required."
        -working: true
        -agent: "testing"
        -comment: "✅ COMPREHENSIVE TESTING COMPLETE (8/8 TESTS PASSED - 100% SUCCESS RATE): Confirmed videoPresentation is NO LONGER required for femmes. Test results with exact HTTP status codes: (1) POST /api/seed -> HTTP 200 {ok:true} ✅ (2) Register FEMME (unique email femme_video_removed_*@maalove.test) -> HTTP 200, status='documents_requis' ✅ (3) POST /api/verification/documents WITHOUT video and WITHOUT pieceIdentite {moyenPaiement:'MoMo Money', referencePaiement:'MP99', forfaitMois:6, forfaitCoach:true, forfaitMontant:90000} -> HTTP 200, status changed to 'en_verification', GET /api/me confirms status='en_verification' ✅ (4a) NEGATIVE TEST: POST /api/verification/documents with empty payload {} -> HTTP 400 with error 'Moyen de paiement et référence/capture requis' (validation enforced) ✅ (4b) NEGATIVE TEST: POST /api/verification/documents with only {moyenPaiement:'Orange Money'} (no ref & no preuve) -> HTTP 400 with error 'Moyen de paiement et référence/capture requis' (validation enforced) ✅ (5) Admin GET /api/admin/verifications -> HTTP 200, femme present in list with forfaitMois==6, forfaitCoach==true, forfaitMontant==90000 (all forfait fields correct) ✅ (6) REGRESSION: Men selfie flow complete - register homme (status 'en_attente'), GET /discover returns 403 (blocked), POST /verification/selfie (status 'en_verification'), GET /discover still 403 (waiting admin), admin verify (status 'verifie'), GET /discover returns 200 with 13 profiles (unlocked) ✅ (7) REGRESSION: GET /api/conversations called 3 times -> all HTTP 200 (no 500 errors, MongoDB connection stable) ✅. The CHANGE is fully functional and working correctly. videoPresentation is no longer required or validated for femmes. pieceIdentite is also not required. Only moyenPaiement + (referencePaiement OR preuvePaiement) + forfait fields are required. All validation rules working as expected. All regression tests passed. No issues found."

    -agent: "testing"
    -message: "✅ VIDEO REMOVAL CHANGE TESTING COMPLETE - ALL 8 TESTS PASSED (100% SUCCESS RATE). Comprehensive testing confirms the CHANGE: videoPresentation is NO LONGER required for femmes in /api/verification/documents (video removed entirely from woman flow). Test results with exact HTTP status codes: (1) POST /api/seed -> HTTP 200 {ok:true} ✅ (2) Register FEMME (unique email) -> HTTP 200, status='documents_requis' ✅ (3) POST /api/verification/documents WITHOUT video and WITHOUT pieceIdentite {moyenPaiement:'MoMo Money', referencePaiement:'MP99', forfaitMois:6, forfaitCoach:true, forfaitMontant:90000} -> HTTP 200, status='en_verification', GET /api/me confirms ✅ (4a) NEGATIVE: empty payload {} -> HTTP 400 'Moyen de paiement et référence/capture requis' ✅ (4b) NEGATIVE: only {moyenPaiement:'Orange Money'} -> HTTP 400 'Moyen de paiement et référence/capture requis' ✅ (5) Admin GET /api/admin/verifications -> HTTP 200, femme present with forfaitMois==6, forfaitCoach==true, forfaitMontant==90000 ✅ (6) REGRESSION: Men selfie flow complete (register -> selfie -> en_verification -> admin verify -> verifie -> discover 200 with 13 profiles) ✅ (7) REGRESSION: GET /api/conversations x3 -> all HTTP 200 (no 500 errors) ✅. The CHANGE is fully functional. Video is no longer required for femmes. All validation rules working correctly. All regression tests passed. No issues found."
